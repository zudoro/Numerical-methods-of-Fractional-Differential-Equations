...
!--------------------------------------------------------------------------------------------!
	...
	DO I = 1, N
		...
		IF(I.EQ.1) THEN
			...
			AN = ALF/(1.0D0*N)
			BN = BET/(1.0D0*N)
			R1 = (1.0D0+ALF)*(2.780D0/(4.0D0 + (1.0D0*N)*N) + 0.7680D0*AN/(1.0D0*N))
		...
		ELSE IF(I.EQ.2)THEN
			...
			R2 = 1.0D0 + 0.060D0*(N-8.0D0)*(1.+0.120D0*ALF)/(1.0D0*N)
			R3 = 1.0D0 + 0.0120D0*BET*(1.0D0+0.250D0*DABS(ALF))/(1.0D0*N)
			Z=Z-(1.0D0-Z)*R1*R2*R3
		ELSE IF(I.EQ.3)THEN
			! INITIAL GUESS FOR THE THIRD LARGEST ROOT.
			R1 = (1.670D0 + 0.280D0*ALF)/(1.0D0 + 0.370D0*ALF)
			R2 = 1.0D0 + 0.220D0*((1.0D0*N)-8.0D0)/(1.0D0*N)
			R3 = 1.0D0 + 8.0D0*BET/((6.280D0+BET)*(1.0D0*N)**2)
			Z = Z-(X(1)-Z)*R1*R2*R3
		ELSE IF(I.EQ.N-1)THEN
			! INITIAL GUESS FOR THE SECOND SMALLEST ROOT.
			R1 = (1.0D0+0.2350D0*BET)/(0.7660D0 + 0.1190D0*BET)
			R2 = 1.0D0/(1.0D0 + 0.6390D0*((1.0D0*N)-4.0D0)/(1.0D0 + 0.710D0*((1.0D0*N)-4.0D0)))
			R3 = 1.0D0/(1.0D0 + 20.0D0*ALF/((7.50D0 + ALF)*(1.0D0*N)**2))
			Z = Z+(Z-X(N-3))*R1*R2*R3
		ELSE IF(I.EQ.N)THEN
			! INITIAL GUESS FOR THE SMALLEST ROOT.
			R1 = (1.0D0 + 0.370D0*BET)/(1.670D0 + 0.280D0*BET)
			R2 = 1.0D0/(1.0D0 + 0.220D0*((1.0D0*N)-8.0D0)/(1.0D0*N))
			R3 = 1.0D0/(1.0D0 + 8.0D0*ALF/((6.280D0+ALF)*(1.0D0*N)**2))
		...
		DO ITS = 1, MAXIT
			...
			PP=((1.0D0*N)*(ALF-BET-TEMP*Z)*P1+2.0D0*((1.0D0*N)+ALF)*((1.0D0*N)+BET)*P2)/(TEMP*(1.0D0-Z*Z))
		...
		W(I) = DEXP(GAMMLN(ALF+(1.0D0*N))+GAMMLN(BET+(1.0D0*N))-GAMMLN((1.0D0*N)+1.0D0)- GAMMLN((1.0D0*N)+ALFBET+1.0D0))*TEMP*2.0D0**ALFBET/(PP*P2)
	ENDDO
