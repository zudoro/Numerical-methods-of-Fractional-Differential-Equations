...
! 	REAL*8, PARAMETER :: BETA  = -0.50D0
	...
	INTEGER, PARAMETER :: NUMIC = 2
	REAL*8, DIMENSION(1:NUMIC), PARAMETER :: IC = (/0.0D0, 0.0D0/)
	
	! Parameters of the Gauss-Jacobi integration
	REAL*8, PARAMETER :: ALF = NU-1.0D0
	REAL*8, PARAMETER :: BET = 0.0D0
...
! 	INTEGER, ALLOCATABLE :: SUBIRNDX(:,:,:)
	...
	! GAUSS POINTS
	INTEGER :: NUMGSPT(4), INNER_NUMGSPT
	REAL*8, ALLOCATABLE :: GSPT(:,:), GSW(:,:), GJPT(:), GJW(:), INNER_GSPT(:)
	REAL*8, ALLOCATABLE :: REFGSPT(:), REFGSW(:), SPGSPT(:,:), SPGSW(:,:), INNER_GSW(:)
...
! 	INTEGER, ALLOCATABLE :: CARRAY(:,:)
	...
	CHARACTER(LEN=3) :: CHAR_NUMPATCH, CHAR_PATCH
	CHARACTER(LEN=1) :: CHAR_PROBLEM
	CHARACTER(LEN=100) :: FILENAME, STIFNAME, LOADNAME, SOLNAME, EXTRANAME
	CHARACTER(LEN=100) :: CHAR_MATRIX, CHAR_LOADF, CHAR_SOL
