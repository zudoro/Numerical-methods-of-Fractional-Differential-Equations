...
TYPE(FUNCTION_1D) FUNCTION GET_PHY_JACOBI_POLY(PHYPT,INDX,PATCH)
	...
	! Conditions needed when PU-FDE is applied
...
! 	ENDIF
	...
	F_INV%VAL(0) = (PHYPT - LEFT_PT)/(RIGHT_PT - LEFT_PT)
	F_INV%VAL(1) = 1.0D0/(RIGHT_PT - LEFT_PT)
	F_INV%VAL(2) = 0.0D0
	
	P = JACOBI_POLY(F_INV%VAL(0), INDX, 1.0D0)
...
TYPE(FUNCTION_1D) FUNCTION JACOBI_POLY(X, INDX, L, FD_SWITCH)
...
! 		ENDIF
! 		
! 		P%VAL(0) = P%VAL(0) + (NUM/DEN)*X**K
! 		
! 		DO J = 0, MAX_DIFF_ORDER - 1
! 			IF (K>=(J+1)) THEN 
! 				P%VAL(J+1) = P%VAL(J+1) + (NUM/DEN)*(DEXP(GAMMLN(K*1.0D0 + 1.0D0))/DEXP(GAMMLN(K*1.0D0 - J*1.0D0)))*X**(K-(J+1))
! 			ENDIF
! 		ENDDO
! 	ENDDO
! 	
